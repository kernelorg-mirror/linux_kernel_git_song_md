			      ...
			      char *secstrings, struct module *me)
{
	...
	me->mod_core_text.size = ALIGN(me->mod_core_text.size, 4);
	me->arch.got_offset = me->mod_core_text.size;
	me->mod_core_text.size += me->arch.got_size;
	me->arch.plt_offset = me->mod_core_text.size;
	if (me->arch.plt_size) {
		if (IS_ENABLED(CONFIG_EXPOLINE) && !nospec_disable)
			me->arch.plt_size += PLT_ENTRY_SIZE;
		me->mod_core_text.size += me->arch.plt_size;
	}
	return 0;
}
		      ...
		      void *(*write)(void *dest, const void *src, size_t len))
{
	...
	case R_390_GOTPLTENT:	/* 32 bit rel. offset to jump slot >> 1. */
		if (info->got_initialized == 0) {
			Elf_Addr *gotent = me->mod_core_text.base +
					   me->arch.got_offset +
					   info->got_offset;
		...
		if (r_type == R_390_GOT12 ||
			...
			val += (Elf_Addr) me->mod_core_text.base - loc;
			rc = apply_rela_bits(loc, val, 1, 32, 1, write);
		}
		break;
	case R_390_PLT16DBL:	/* 16 bit PC rel. PLT shifted by 1.  */
	...
	case R_390_PLTOFF64:	/* 16 bit offset from GOT to PLT. */
		if (info->plt_initialized == 0) {
			...
			plt_base = me->mod_core_text.base + me->arch.plt_offset;
		...
		if (r_type == R_390_PLTOFF16 ||
			...
			if (!((r_type == R_390_PLT16DBL &&
				...
				val = (Elf_Addr) me->mod_core_text.base +
					me->arch.plt_offset +
					info->plt_offset;
			val += rela->r_addend - loc;
		}
	...
	case R_390_GOTOFF64:	/* 64 bit offset to GOT. */
		val = val + rela->r_addend -
			((Elf_Addr) me->mod_core_text.base + me->arch.got_offset);
	...
	case R_390_GOTPCDBL:	/* 32 bit PC rel. off. to GOT shifted by 1. */
		val = (Elf_Addr) me->mod_core_text.base + me->arch.got_offset +
			rela->r_addend - loc;
		    ...
		    struct module *me)
{
	...
	if (IS_ENABLED(CONFIG_EXPOLINE) &&
		...
		ij = me->mod_core_text.base + me->arch.plt_offset +
			me->arch.plt_size - PLT_ENTRY_SIZE;
