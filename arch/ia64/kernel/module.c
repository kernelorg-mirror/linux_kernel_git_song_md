...
{
	return addr - (uint64_t) mod->mod_init_text.base < mod->mod_init_text.size;
}

static inline bool
in_core (const struct module *mod, uint64_t addr)
{
	return addr - (uint64_t) mod->mod_core_text.base < mod->mod_core_text.size;
}
	  ...
	  Elf64_Shdr *sec, void *location)
{
	...
	switch (formula) {
		...
		break;
	      ...
	      case RV_BDREL:
		val -= (uint64_t) (in_init(mod, val) ? mod->mod_init_text.base :
				   mod->mod_core_text.base);
		break;
		    ...
		    unsigned int relsec, struct module *mod)
{
	...
	if (!mod->arch.gp) {
		...
		if (mod->mod_core_data.size > MAX_LTOFF)
			/*
			 * This takes advantage of fact that SHF_ARCH_SMALL gets allocated
			 * at the end of the module.
			 */
			gp = mod->mod_core_data.size - MAX_LTOFF / 2;
		else
			gp = mod->mod_core_data.size / 2;
		gp = (uint64_t) mod->mod_core_data.base + ((gp + 7) & -8);
		mod->arch.gp = gp;
